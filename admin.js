...
function openMember(id){
  const m=members.find(x=>x.id===id);$('memberTitle').textContent=m?'회원 수정':'회원 추가';$('memberId').value=m?.id||'';$('memberName').value=m?.name||'';$('memberAge').value=m?.age||'';$('memberPhone').value=m?.phone||'';$('memberPosition').value=m?.position||'공격';$('memberPin').value='';$('memberPin').placeholder=m?'변경할 때만 새 PIN 입력':'4자리 숫자';openModal('memberModal')
}
async function saveMember(){
  ...
  const payload={name,age:Number($('memberAge').value)||null,phone:$('memberPhone').value.trim()||null,position:$('memberPosition').value};
  let q=id?sb.from('members').update(payload).eq('id',id):sb.rpc('admin_create_member',{p_name:name,p_age:payload.age,p_phone:payload.phone,p_position:payload.position,p_pin:pin});
...
function renderMembers(){
  ...
  $('memberBody').innerHTML=rows.map(m=>`<tr><td><b>${m.name}</b></td><td>${m.age||'-'}</td><td>${m.phone||'-'}</td><td>${badge(m.position)}</td><td>••••</td><td><button class="btn small" onclick="openMember('${m.id}')">수정</button> <button class="btn red small" onclick="deleteMember('${m.id}')">삭제</button></td></tr>`).join('')||'<tr><td colspan="6" class="empty">회원 없음</td></tr>';
}
