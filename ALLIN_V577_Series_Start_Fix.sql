...
begin
  ...
  if v_attendee_count between 9 and 11 and p_referee_member_id is null then
    raise exception '참석 인원이 9~11명일 때에는 심판을 지정하세요.';
  end if;

  if (v_attendee_count<=8 or v_attendee_count>=12)
     and p_referee_member_id is not null then
    raise exception '현재 참석 인원에서는 별도 심판을 지정하지 않습니다.';
  end if;
